...
export const SHOP_CARD_CSS = `
  .psc {
    width: 100%; max-width: 26.25rem; margin: 0 auto;
  ...
  .psc-head { padding: 1.25rem 1.38rem 1.12rem; display: flex; gap: 0.88rem; align-items: flex-start; }
  .psc-head h3 { margin: 0; font-size: 1.19rem; font-weight: 600; letter-spacing: -0.02em; line-height: 1.2; }
  .psc-head .when { font-size: 0.78rem; color: var(--ink-3); margin-top: 0.31rem; display: flex; flex-direction: column; gap: 3px; }
  .psc-head .when span { display: inline-flex; align-items: center; gap: 0.38rem; }
  .psc-datechip {
    width: 3.25rem; flex-shrink: 0; border: 1px solid var(--line);
    border-radius: 9px; overflow: hidden; text-align: center; background: var(--surface);
  }
  .psc-datechip .m {
    font-size: 0.59rem; letter-spacing: 0.1em; color: #fff; text-transform: uppercase;
    font-weight: 600; background: var(--accent); padding: 3px 0;
  }
  .psc-datechip .d { font-size: 1.25rem; font-weight: 600; padding: 0.25rem 0 0.31rem; letter-spacing: -0.02em; font-variant-numeric: tabular-nums; }
  .psc-desc { padding: 0 1.38rem 1.12rem; font-size: 0.81rem; color: var(--ink-2); line-height: 1.6; white-space: pre-line; }
  .psc-rows { border-top: 1px solid var(--line); padding: 1rem 1.38rem; display: flex; flex-direction: column; gap: 0.75rem; }
  .psc-row { display: flex; align-items: center; justify-content: space-between; gap: 0.75rem; font-size: 0.84rem; flex-wrap: wrap; }
  .psc-row .k { color: var(--ink-3); min-width: 0; }
  .psc-row .k .sub { display: block; font-size: 0.69rem; color: var(--ink-4); margin-top: 2px; }
  .psc-row .v { font-weight: 600; font-variant-numeric: tabular-nums; min-width: 0; }
  .psc-row .v.big { font-size: 1.19rem; letter-spacing: -0.01em; }
  .psc-foot { border-top: 1px solid var(--line); padding: 1.12rem 1.38rem 1.25rem; background: var(--surface-2); display: flex; flex-direction: column; gap: 0.5rem; }
  .psc-tierrow { display: flex; justify-content: space-between; gap: 0.75rem; font-size: 0.81rem; color: var(--ink-2); }
  .psc-tierrow b { font-variant-numeric: tabular-nums; }
  .psc-cta {
    margin-top: 0.5rem; height: 2.62rem; border-radius: 10px;
    background: var(--accent); color: #fff; display: grid; place-items: center;
    font-size: 0.88rem; font-weight: 600; opacity: 0.75;
  }
`;
